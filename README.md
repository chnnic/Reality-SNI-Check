...
> 一次跑完，自动测出**全合规优先**且握手较快、满足 Reality 硬性要求（TLS1.3 + h2 + X25519 + 短证书链 + 证书有效）的候选站点。

选 SNI 不是拍脑袋填一个大站就完事——同一个站在不同落地机上的握手延迟差异很大，而且有些站根本不满足 Reality 的握手条件（比如不支持 TLS1.3、证书链太长、证书无效）。这个脚本把这些判断自动化：**批量探测 → 合规筛选 → 全合规优先排序 → 直接告诉你这台机该用哪个。**
...
- **Reality 合规检测**：基于 `openssl` 拿到 **TLS1.3 / ALPN(h2) / 临时密钥(X25519) / 证书链层数 / 证书验证结果**，不合规的站直接标记。
- **SNI 风险避坑**：内置候选已剔除 GFWList 整域名规则命中的常见高风险 SNI；自定义输入仍会用内置 GFWList 后缀表检测并提示 `△ SNI风险`。
- **证书链长度检查**：链太长（4+ 层）会导致 Reality 借用握手时出问题，脚本自动标出；`www.microsoft.com` 这类已知链风险站已从内置候选剔除。
- **10 大分类、98 个内置站点**：覆盖 CDN、云、科技、金融、大学、流媒体、社交、游戏、电商、区域锚点，可按需选测。
- **自动排名 + 推荐**：全合规候选（TLS1.3 + h2 + X25519 + 短链 + 证书有效）优先，组内再按握手升序；没有全合规候选时才给备选。
...
| `gaming` | 游戏平台 | 8 |
| `ecom` | 电商 / 零售 / 品牌 | 10 |
| `region` | 区域锚点（日 / 欧 / 东南亚 / 港 / 澳） | 11 |

跑全部 98 站会花点时间；日常按落地机大区选 2~3 个分类即可，例如新加坡落地机跑 `-c cdn,cloud,edu`。
...
如果没有全合规候选，脚本会改为输出 `→ 无全合规候选，备选 dest/SNI: ...`，表示该站只适合作为退而求其次的候选，不是首选。
...
证书验证结果不会单独占一列；证书无效会在“合规”列显示 `✗ 证书`，并从推荐排名中排除。自定义输入命中内置 GFWList 后缀表时会显示 `△ SNI风险`，同样不进入默认推荐排名。

### 合规标记

| 标记 | 含义 |
|------|------|
| `✓ 推荐` | TLS1.3 + h2 + X25519 + 短链（≤3）+ 证书有效全部满足，最佳 dest |
| `△ 链偏长` | TLS1.3 / h2 / X25519 / 证书有效通过，但证书链 4+ 层，能试但不首选 |
| `△ 可用` | 通过 TLS1.3，但缺 h2 或 X25519 |
| `△ SNI风险` | TLS 参数可能合规，但命中内置 GFWList 后缀表，不进入默认推荐 |
| `✗ 链风险` | 已知 Reality/VLESS 证书链兼容性风险，不推荐 |
| `✗ 证书` | 证书无效或主机名校验失败，不推荐 |
...
- **证书有效性**：目标站证书应通过 CA 与主机名校验；证书异常会降低伪装可信度，脚本不会推荐。
- **SNI 风险**：部分大站虽然从落地机方向 TLS 全合规，但其 SNI 在中国大陆网络环境下风险较高；内置候选已剔除 GFWList 整域名规则命中的风险站，自定义输入会按内置 GFWList 后缀表提示风险。
...
**Q：`www.microsoft.com` 为什么不在内置候选里？**
A：它现在不适合作为 VLESS-Reality 的默认候选：部分区域返回的证书链偏长（含多级/交叉签名），Reality 借用握手易出问题。脚本已从内置列表剔除它；如果手动输入，会标记为 `✗ 链风险` 并排除推荐。
