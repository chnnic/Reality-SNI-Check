...
- **交互菜单**：直接运行即进入菜单，数字选择测试项（快速测试 / 全部 / 选分类 / 按地区推荐 / 自定义 / 设置），无需记参数。
- **按地区推荐**：给站点打了归属地区标签（日 / 港 / 东南亚 / 欧 / 美等），可按落地机所在大区筛「就近归属 + 全球通用」的候选，SNI 更贴合正常流量画像。
...
- **4 按地区推荐**：选一个地区（日本 / 香港 / 东南亚 / 欧美等），脚本测「该地区归属站 + 全球通用站」，推荐里挑就近又合规的——落地机在哪个区就选哪个，SNI 更贴合正常流量画像。
- **5 自定义网址检测**：手动输入网址，一行可输入多个（逗号或空格分隔）。
- **6 设置**：调整每站探测次数与单次超时。
- **0 退出**。
...
./Reality-SNI-Check.sh -r jp            # 按地区推荐（日本归属站 + 全球站）
...
| `-r <地区>` | 按地区推荐：测该地区归属站 + 全球站（`global us eu jp hk sg cn au kr`） |
